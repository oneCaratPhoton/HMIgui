...
{                                                                 
    rt_uint16_t _head;                                                       //队列头
    rt_uint16_t _tail;                                                       //队列尾
    rt_uint8_t _data[QUEUE_MAX_SIZE];                                       //队列数据缓存区
}QUEUE;                                                           

static QUEUE que = {0,0,0};                                            //指令队列
static rt_uint32_t cmd_state = 0;                                           //队列帧尾检测状态
static rt_uint16_t cmd_pos = 0;                                              //当前指令指针位置
...
{
    rt_uint16_t pos = (que._head+1)%QUEUE_MAX_SIZE;
...
//从队列中取一个数据
static void queue_pop(rt_uint8_t* _data)
{
...
*/
rt_uint16_t queue_find_cmd(rt_uint8_t *buffer,rt_uint16_t buf_len)
{
    rt_uint16_t cmd_size = 0;
    rt_uint8_t _data = 0;
